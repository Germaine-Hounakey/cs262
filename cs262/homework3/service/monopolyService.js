...
// Homework3 changes
router.get("/joinTables", joinTables);
...
// Homework3 changes
function joinTables(req, res, next) {
    db.many(
      "SELECT Game.ID, name, score FROM Game, Player, PlayerGame WHERE Player.ID = playerID AND gameID = Game.ID ORDER BY score DESC"
    )
      .then((data) => {
        res.send(data);
      })
